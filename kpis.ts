...
/** Cuenta propiedades (sin duplicar) por categoria General/DDC/Extra. */
export function countByCategoria(properties: PropertyLike[]): Record<Categoria, number> {
  const counts: Record<Categoria, number> = { General: 0, DDC: 0, Extra: 0 };
  for (const p of properties) {
    counts[classifyCategoria(p.agrupacion)] += 1;
  }
  return counts;
}

/**
 * Dias promedio entre la fecha de ingreso y la fecha en que dejo de estar
 * en venta/alquiler (detectada por el sistema, no viene de NAI). Solo cuenta
 * propiedades que ya se cerraron desde que esta funcionalidad esta activa -
 * no hay datos retroactivos de cierres anteriores.
 */
export function avgDiasHastaCierre(
  properties: PropertyLike[],
  tipo: "venta" | "alquiler"
): { promedio: number | null; cantidad: number } {
  const campo = tipo === "venta" ? "fecha_cierre_venta" : "fecha_cierre_alquiler";
  const dias: number[] = [];

  for (const p of properties) {
    const cierre = p[campo];
    if (!cierre) continue;
    const ingreso = parseNaiDate(p.fecha_ingreso);
    if (!ingreso) continue;
    const cierreDate = new Date(cierre);
    const diff = (cierreDate.getTime() - ingreso.getTime()) / (1000 * 60 * 60 * 24);
    if (diff >= 0) dias.push(diff);
  }

  return {
    promedio: dias.length ? dias.reduce((a, b) => a + b, 0) / dias.length : null,
