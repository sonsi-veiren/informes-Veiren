...
interface Property {
  ...
  fecha_cierre_alquiler: string | null;
}
...
export default function HomePage() {
  ...
  const categoriaCounts = useMemo(() => countByCategoria(properties), [properties]);
  const diasVenta = useMemo(() => avgDiasHastaCierre(properties, "venta"), [properties]);
  const diasAlquiler = useMemo(() => avgDiasHastaCierre(properties, "alquiler"), [properties]);
  ...
  async function exportPdf() {
    ...
    try {
      ...
      pdf.save(`informe-veiren-${fecha}.pdf`);
    } finally {
  ...
  return (
    <main style={{ padding: "24px 32px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div ref={pdfRef} style={{ background: "#eeeeee" }}>
        {/* KPIs */}
        <section
          style={{
            display: "grid",
        ...
        >
          <KpiCard
            ...
            sub={`DDC: ${categoriaCounts.DDC} · Externo: ${categoriaCounts.Extra} · General: ${categoriaCounts.General}`}
          />
          ...
          />
          <KpiCard
            label="Días prom. hasta vender"
            value={diasVenta.promedio !== null ? `${Math.round(diasVenta.promedio)} días` : "Sin datos aún"}
            sub={diasVenta.cantidad ? `Basado en ${diasVenta.cantidad} cierre(s) detectados` : "Se va acumulando con cada sync"}
          />
          <KpiCard
            label="Días prom. hasta alquilar"
            value={diasAlquiler.promedio !== null ? `${Math.round(diasAlquiler.promedio)} días` : "Sin datos aún"}
            sub={diasAlquiler.cantidad ? `Basado en ${diasAlquiler.cantidad} cierre(s) detectados` : "Se va acumulando con cada sync"}
          />
        </section>
        ...
        >
          ...
          />
          <GroupedCategoriaChart data={categoriaOperacionData} label="General vs DDC vs Externo — por operación" />
        </section>
      </div>

      {/* Buscador + Código de edición */}
      <div
        style={{
          display: "flex",
          flexWrap: "wrap",
      ...
      >
        <div>
          ...
          <input
            ...
            placeholder="Título, ubicación, agente, tipo..."
            className="editable-input"
            style={{ width: 260 }}
          />
          {searchQuery && (
            <span style={{ color: "#383b43", marginLeft: 8 }}>
      ...
      {loading ? (
        ...
        <div className="table-scroll-wrapper" style={{ border: "1px solid #d9d9d9", borderRadius: 6 }}>
          <table>
            <thead>
              <tr>
                <th>ID</th>
...
function KpiCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div style={{ background: "white", borderRadius: 8, padding: "14px 16px", border: "1px solid #d9d9d9" }}>
      ...
      {sub && <div style={{ fontSize: 11, color: "#8891a5", marginTop: 2 }}>{sub}</div>}
    </div>
  );
}
